...
const EmptyStates: React.FC<EmptyStatesProps> = ({
   ...
   return (
      <div className={tw('grid w-full gap-4', classNames?.base)}>
         <svg
            className='mx-auto'
         ...
         <div>
            <h2
               className={tw(
                  'pb-1 text-center text-base font-semibold leading-relaxed text-black',
                  classNames?.title
               )}
            >
               {title}
            ...
            {subtitle && (
               <p
                  className={tw(
                     'pb-4 text-center text-sm font-normal leading-snug text-black',
                     classNames?.subtitle
                  )}
               >
                  {subtitle}
            ...
            {children && <div className={tw('flex gap-3', classNames?.action)}>{children}</div>}
         </div>
      </div>
   );
};
