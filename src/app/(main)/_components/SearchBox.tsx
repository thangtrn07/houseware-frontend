...
import { LoaderCircle, Search, SearchIcon, Store } from "lucide-react";
...
const SearchBox = () => {
   ...
   const {
      ...
      isLoading,
      isFetched,
   } = useQuery<SearcgQueryData>({
      queryKey: ["search-suggestion", searchText],
      queryFn: () =>
         fetchProductSearch({ name: searchText, page: 1, limit: 10 } as any),
               ...
               >
                  <ul className="max-h-[80vh] overflow-y-auto py-2 text-sm">
                     <div className="flex items-center overflow-hidden text-nowrap px-4 py-2 transition-all ease-linear">
                        ...
                        <span className="truncate font-bold text-[--red-color]">
                           {searchText}
                        </span>
                        <span>&quot;</span>
                     </div>
                     {searchText.length > 0 && (
                        <>
                           <Divider />
                           {isLoading && (
                              <div className="transition-linear flex items-center px-4 py-2 justify-center">
                           ...
                           {isFetched &&
                           !isLoading &&
                           !searchProductData?.result ? (
                              <div className="transition-linear flex items-center px-4 py-2 hover:bg-[--gray-300-color]">
